...
INSERT INTO `s_askreport` (`ID`, `TITLE`, `PERSON_ID`, `SENDER_NAME`, `SEND_DATE`, `ASK_DATE`, `SENDER_DP`, `ASKREPORT_CONTENT`, `REC_ID`, `REC_NAME`, `ACCEPT_DATE`, `IS_VIEW`, `STATE`, `DELETEID`, `FILE_ID`) VALUES
	(1, '买铅笔', 1, '李明明', '2016-05-26 15:24:02', '2016-05-26 15:24:14', '教务处', '现在需要铅笔', 15, '王凯', '2016-05-26 16:03:36', 1, 0, 1, NULL);
...
CREATE TABLE IF NOT EXISTS `s_notify` (
  ...
  `TITLE_TOP_FLAG` int(2) DEFAULT '0' COMMENT '是否置顶，默认0未置顶，1置顶',
  `NOTIFY_TYPE` varchar(50) DEFAULT NULL COMMENT '公告类型，默认1，现在也不知道有什么类型',
  `FILE_ID` int(10) DEFAULT NULL COMMENT '附件文件名编号',
  PRIMARY KEY (`ID`)
) ENGINE=MyISAM AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COMMENT='存储通告信息';

-- 正在导出表  db_saes.s_notify 的数据：2 rows
/*!40000 ALTER TABLE `s_notify` DISABLE KEYS */;
INSERT INTO `s_notify` (`ID`, `TITLE`, `SENDER_ID`, `SEND_DATE`, `CONTENT`, `VIEW_TIMES`, `TITLE_TOP_FLAG`, `NOTIFY_TYPE`, `FILE_ID`) VALUES
	(1, '刘延东副总理视察上海理工大学', 1, '2015-12-11 09:00:05', '12月6日，中共中央政治局委员、国务院副总理刘延东来到上海理工大学视察上海太赫兹波谱与影像技术协同创新中心建设和发展情况。中共中央政治局委员、上海市委书记韩正，教育部部长袁贵仁，国务院副秘书长江小涓，上海市副市长翁铁慧，上海市教委主任苏明等陪同视察。', 1, 1, NULL, NULL),
	(2, '关于学校2015年在职教职工体检通知\r\n', 1, '2015-12-11 09:00:49', '校内各部门：', 1, 0, NULL, NULL);
...
CREATE TABLE IF NOT EXISTS `s_notify_view_info` (
  ...
  `IS_VIEW` int(11) DEFAULT '0' COMMENT '是否查看，0未查看，1已查看',
  PRIMARY KEY (`ID`)
