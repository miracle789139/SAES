 ...
 */
@Repository
public class NotifyDao extends AbstractBaseDao {
    ...
    public List<Map<String, Object>> getListByPage(String send_id, String getType, int start, int numberPage) {
        if (getType.equals("all")) {
           ...
           /*发送人，优先级1，什么状态都有可能；带个参数决定吧，默认先这个*/
            sql = "select *  from s_notify s where s.SENDER_ID = ? and s.DELETEID= 0  order by s.id  desc limit ?,?;";
        } else {//appro
            /*待审核人，优先级2，状态为0的*/
    ...
    public int findTotalRecordNum(String send_id, String getType) {
        if (getType.equals("all")) {
            /*接收人，也发给了自己，其实也就是状态=1*/
            sql = "select count(*) from s_notify a left join s_notify_view_info b on a.id = b.NOTIFY_ID where b.rec_id = ? and a.DELETEID=0 and a.STATE = 1";
        } else if (getType.equals("sender")) {
           /*发送人，优先级1，什么状态都有可能；带个参数决定吧，默认先这个*/
            sql = "select count(*)  from s_notify s where s.SENDER_ID = ? and s.DELETEID= 0";
        } else {//appro
            /*待审核人，优先级2，状态为0的*/
